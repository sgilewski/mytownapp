...
export async function getChamberDashboard(){
  ...
  if(auth.demo)return{metrics:demoChamberMetrics,businesses:demoBusinesses,events:demoEvents,invitations:[],chamberId:"c1",isDemo:true};
  const chamberIds=auth.memberships.map(m=>m.chamber_id).filter((id):id is string=>Boolean(id));
  const [{data:chambers},{data:events},{data:invitations}]=await Promise.all([
  ...
  const metrics:DashboardMetric[]=[
    ...
    {label:"Team activity",value:String((events?.length??0)+(invitations?.length??0)),change:"managed records",trend:"neutral"}
  ];
  return{metrics,businesses:businesses??[],events:events??[],invitations:invitations??[],chamberId:chamberIds[0]??"",isDemo:false};
}
